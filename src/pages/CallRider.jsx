...
function CallRider() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setUserCoords([51.505, -0.09]); // fallback coordinates
        }
      );
    } else {
  ...
  // Simulate rider starting position relative to user
  const riderCoords = userCoords
    ? [userCoords[0] + 0.01, userCoords[1] - 0.01]
    : [51.515, -0.1];
  const routeCoords = [userCoords || [51.505, -0.09], riderCoords];
    ...
    <div className="relative w-full h-screen">
      ...
      >
        ...
        <Marker position={riderCoords} icon={riderIcon}>
          <Popup>Rider's Starting Position</Popup>
        </Marker>
        {/* Dashed polyline for an attractive route */}
        <Polyline positions={routeCoords} color="#FFBF00" dashArray="8, 8" />
      </MapContainer>
